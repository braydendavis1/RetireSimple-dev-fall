...
namespace RetireSimple.NewEngine.New_Engine.GrowthModels._401kGrowthModels {
	public class _401kGrowth : IGrowthModel {
		...
		private Projection DoGenerateProjection(double value, int years, _401kInfoModel info) {
		...
		public Projection GenerateProjection(double value, int years, InvestmentVehicleInfoModel info) {
			return this.DoGenerateProjection(value, years, (_401kInfoModel)info);

		}
		private double CalculateEmployerContributions(_401kInfoModel info, int i) {
		...
		private double CalculateSalaryIncrease(_401kInfoModel info, int i) {
